...
quasihalt.
  ...
  [SUPERSEDED by §2b4 — read that instead of acting on this ranking.  Move (2)
  is DONE.  Move (1) is wrong in its diagnosis: those eleven are FIBONACCI
  counters, not base-2 nested ones, so the `nestcert` head symbol is not what
  blocks them.]

## 2b4. Wave-36 (2026-07-31) — the `StA` row boards; the rest count in PHI

Full write-up: `docs/CORE_3STATE.md` §3 (that section was REWRITTEN — the
wave-35 reading of this row was wrong in an instructive way).
**`1RB---_1RC1LB_0LB1RD_0RA0RC` is boarded**:
`Machines/Counters/NLAP_1RB____1RC1LB_0LB1RD_0RA0RC.v`, `NeverQuasiHaltsSt`
via `LapGlue.glue_neverqh` — **and with it its `0RB` shadow**
`0RB0RD_1RC---_1RD1LC_0LC1RA`
(`Machines/Counters/RRNQ_0RB0RD_1RC____1RD1LC_0LC1RA.v`).  Frozen unproven
...
- **A shadow does NOT resolve for free when its core row boards — it INVERTS.**
  ...
  a row in `D_remaining`).  Board the core row and the shadow loses its
  target: `gen_stages` moves it OUT of `shadow_rows.tsv` and INTO
  `core_rows.txt`, so the core count does not drop until the shadow is boarded
  too.  Budget for the pair.  The transport is cheap —
  `Census/Reroot.neverqh_reroot` across the blank prefix, then two
  non-start-state relabellings — but it needs one general lemma the tree did
  not have: **never-quasihalting is invariant under `TM_swap u v` for
  `u, v <> StA`** (`neverqh_swap`, the same argument as
  `Reroot.visitsat_reroot` with `es_swap_init` in place of the prefix
  identity).  It is kept LOCAL to the shadow board on purpose: `Census/Reroot.v`
...
- **The wave-35 diagnosis was wrong, and the correction is the reusable
  ...
  head.  It is not, and no such template was needed.  Restrict that same
  family to its `j = 0` members — carry run EMPTY — and the anchor is
  ORDINARY: `Cin v = (StB, (S1 :: Rw v, S0, [S0]))`, head at the right end of
  the written region, whole word to its left, and consecutive `j = 0` anchors
  are consecutive values.
- **What actually hid it: every anchor search in the tree scores a candidate
  family by its longest run of CONSECUTIVE VALUES.**  This counter is
  FIXED-WIDTH inside an epoch (`v` runs `2^(2i) .. 2^(2i+1) - 1`, then the
  word widens and `v` JUMPS to `2^(2i+2)`), so no scan can ever see a long
  consecutive run, and `residue_map.tsv` called it "no-anchor / no overflow
  phase at K=6" for four waves.  The head-symbol `HD` hole wave-35 opened is
  real but was not this row's blocker — its anchor head is `S0`.
- **`tools/counters/nestscan.py`** (new) is the fix: it scores a family by the
  LOCAL `Alph` increment structure of ADJACENT anchor words
  (`w = pre B^j A tail`, `w' = pre A^j B tail`), voting `(A, B, |pre|)` out of
  the pairs, with no notion of "the value".  It reports this row at
  `run = 255` and prints the word-length breaks, which ARE the epoch
  boundaries.  Two details cost a family if dropped: the `pre` (the stop
  digit's marker sits in front of the run) and a separate `j = 0` arm (there
  `B` does not occur in `w` and must be read from `w'`).  **Run this before
  calling any row "no-anchor".**
- **Proof shape**: `Counters/NestedLap.v` with the EPOCH as the outer level —
  outer anchor `Cc p := Cin (pow2 (2 * Pos.to_nat p))`, boot = one interior
  lap (8), inner = `inner_to_fill` (the induction, cost existential), exit =
  the widening (`8j + 11`).  Both laps are `LapDecider` certificates.  `StA`
  fires ONLY on the widening, so its visit witness is a prefix of the EXIT
  chain.  New alphabet `Counters/Alph_01_11_1.v` (`A=01`, `B=11`, `C=1`),
  generated by `gen_alphabet.py`.
- **The remaining eleven are NOT this shape** — `nestscan.py` is negative on
  all eleven (best stretch 1 or 2 pairs at every anchor).  Do not re-chase.

- **THE HEADLINE FOR NEXT SESSION: the eleven are FIBONACCI counters, not
  base 2.**  `tools/counters/radix_clock.py` (new) reads a radix with no
  anchor, no word family and no alphabet — count, per tape cell, how many
  writes actually CHANGED it; a digit of weight `r^k` toggles `~V/r^k` times,
  so the ratio between cells one digit apart IS the radix.  On all eleven:
  **ratio = 1.6180 = phi, spread 0.00 across every digit**, and the raw counts
  `94432, 58362, 36070, 22291, 13776, 8514, 5262, 3252` satisfy
  `F(n) = F(n-1) + F(n-2)` exactly.  Calibrated against three rows whose radix
  is already a theorem here — `KpWallAlt`'s base 2 (spread 0.00), the `StA`
  row's base 2 (stride 2), `Ter3Wall`'s base 3 (spread 0.00) — each recovered
  exactly.  Cross-checked on tape width: 23 cells and `~4.7e4` low-digit
  toggles at `t = 2e5`; `phi^23 ~ 6.4e4` right, `2^23 ~ 8.4e6` wrong by two
  orders.
  That is the whole explanation of four waves of "no-anchor" on these rows:
  **every alphabet in `theories/Counters` is base 2** (plus `Ter3Wall`'s base
  3), so a Fibonacci counter's words cannot decode to consecutive values at
  ANY anchor, which is precisely the "sparse self-similar" value set
  `1 | 3 | 6,7 | 12,13,15 | …` that `residue_map` has been recording.  It is
  wave-35's base-3 lesson one notch further out.
- **…and then the anchor fell out immediately.  It was never the problem.**
  `tools/counters/fib_anchor.py` (new) is the ORDINARY anchor scan — same
  anchor space, same consecutive-value score — with the weight ladder
  `1,1,2,3,5,8,…` in place of `1,2,4,8,16`.  **All eleven decode to
  `0,1,2,3,4,…` at an ordinary flat anchor**, ten under `F(1,1)` and one under
  `F(1,2)`, all at the full 4,000-word cap.  Two checked exhaustively:
  `1RB---_0LB1RC_1LB0RD_1LC0RD` at `(StB, S1, word right, left empty)` —
  **0 failures in 57,300 transitions**; `1RB---_0LC1RD_1LB1RC_1LB0RD` at
  `(StB, S0, word right, 1 cell left)` — **0 in 21,890**.  And the words are
  the ones `docs/CORE_3STATE.md` had already written down —
  `(empty), 1, 11, 011, 111, 0011, 1011, 1111, 00011, …` — the sequence four
  waves called "plain `Kp` words at a sparse self-similar set
  `1|3|6,7|12,13,15|…`".  Weighed as Fibonacci they are `0,1,2,3,4,5,…` with
  nothing skipped.  **The sparse set was a base-2 misreading and nothing
  else.**
  **Sized next piece: a FIBONACCI numeral module**, the analogue of
  `Counters/TernCounter.v`.  Three parts: (i) `Fib v` — and it is an ALPHABET
  FAMILY like `Alph_*`, not one fixpoint, because the two rows sampled use
  different digit words (`1, 11, 011, 111, 0011, …` vs
  `1, 11, 101, 111, 1101, 1011, 1111, …`); (ii) the increment, which folds
  `F(k) + F(k+1) -> F(k+2)`, so `MonoCounter.cview` (a base-2 trailing-ones
  split) does NOT apply and the view must name the Fibonacci-carry position;
  (iii) then the ordinary route — `LapDecider` chains at the anchor above and
  `LapGlueIx`-style glue, since a Fibonacci counter has no `positive` to index
  by, exactly as base 3 did not.
  **The lap law is measured and it is AFFINE.**  With `r` = index of the
  highest digit the increment rewrites, the gap is SINGLE-VALUED in every
  observed class on both rows sampled: `2r` on
  `1RB---_0LB1RC_1LB0RD_1LC0RD` (22 classes, 38,201 laps, one branch), and
  `2r+4` / `2r+8` by parity of `r` on `1RB---_0LC1RD_1LB1RC_1LB0RD`
  (19 classes, 21,890 laps) — the same two-branch shape `Ter3WallB` already
  handles.  So these are FLAT `LapDecider` boards in a different base, not
  nested ones; the "lap is super-affine `6,16,36,82,196`" line in §2b3 was
  measuring the base-2 misreading, where consecutive wall visits span a whole
  sub-epoch.

- **The transferable lesson from BOTH of this wave's findings: the anchor
  space was never the problem, the DECODING was.**  Twice in one session a row
  written off as "no-anchor" turned out to sit at an ordinary flat anchor
  every scan already visits — missed once because the score demanded
  consecutive values from a nested counter, once because the weights were
  `1,2,4,8` on a machine counting in Fibonacci.  **Run `radix_clock.py` FIRST
  on any row called "no-anchor"**: it needs no anchor at all, so it answers
  when every other tool has given up, and it says which machinery can apply.
...
## 2b6. Wave-38, KCOPY3 track (2026-08-01) — the row boards, and the residue map had it filed wrong twice

Row `1RB1RC_1LA1RA_0RC1LD_1LB0LD` — one of the four undecided (4,2) core
rows — boarded as `NeverQuasiHaltsSt`.  **core undecided 4 -> 3, settled
5,151 -> 5,152.**  Full write-up: `docs/LADDER_PLAN.md` §4aa.  The two
things worth carrying out of it:

- **`EXP2` + `no inner family at pow2 j` in `residue_map.tsv` does NOT mean
  the nested/cascade shape.**  This row was queued as `Bin3Lap.v` again and
  is not: the lap is AFFINE in the carry length (`6j+6` / `6j+4` / `6j+8`,
  0 mismatches over 250,012 laps), because the LSB sits against the wall and
  an increment is local.  `EXP2` is the run's growth against the tape width,
  not the shape of a lap.  Run `tools/counters/kc3lap.py`'s lap table before
  reading a residue-map class as a proof shape — it is one command and it
  would have saved the whole nested-route detour.

- **`Alph_*` files generated by `alphabet_infer.py` are verified on as
  little as ONE anchor word, and the header says so.**  This row's real
  numeral packs its top TWO digits into four cells instead of six, so
  `Alph_000_111_111.Ap` mismatches at 250,013 of 250,013 anchor visits.  The
  packing is forced by the extent law (a uniform `3n`-cell word plus a
  marker does not fit in `3n-1` cells), so no re-anchoring reads it away,
  and every uniform-alphabet emitter derives nothing here.  New numeral:
  `theories/Counters/Kc3Num.v`, five clauses.

New tooling, all untrusted: `tools/counters/kc3lap.py` (anchor + lap law
per `cview` class + the `Hvis` liveness count) and `tools/counters/kc3lem.py`
(the three unit rules stated against the probe over an UNKNOWN context,
before any Coq).  `kc3lem.py` is the one that paid: `INT` and `FRT` survive
