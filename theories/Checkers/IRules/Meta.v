(** * IRules.Meta: the certificate checker and the meta-cycle induction.

    A v1 inductive-rules certificate (BBB docs/irules.md) carries a
    template [C(k)] (runs with counts [alpha_i * k + beta_i]), the
    bound [kmin], the meta map [(a, b)], the anchor [(step, k0)] and
    the rules.  The checker:

    1. validates every rule by fresh-variable replay ([rule_check]);
    2. re-simulates the run from the blank tape to [anchor] and
       compares the configuration to [C(k0)] exactly;
    3. replays the meta-cycle symbolically from [C(k)] under
       [k >= kmin], applying rules at first match, and requires it
       to end at [C(a*k + b)] (strict run-list equality);
    4. checks the meta map maps [[kmin, oo)] into itself, and that
       every state visited in the simulated prefix has a transition
       in the cycle's fired set [F].

    Soundness: by induction the machine runs one meta-cycle per
    iterate of [f k = a*k + b] forever, so it never halts; every
    transition in [F] fires in every cycle (so its state recurs
    unboundedly), and every configuration from the anchor on fires a
    transition in [F] (so no other state is ever visited again).
    With check 4 this gives state-level never-quasihalting --
    exactly [NeverQuasiHaltsSt].  (All 428 v1 irules certificates
    denote machines that never quasihalt at state level, including
    the transition-level-QH ones: a state with one finite and one
    infinite transition still recurs.)

    The [fuel] parameter bounds both the rule replays and the
    meta-cycle replay (the C verifier's IVRULEOPS/IVOPS); soundness
    holds for every value, so callers pick something generous. *)

From Coq Require Import Arith ZArith Lia Bool List Setoid.
From BBB4 Require Import BBB4_Statement CTape.
From BBB4.Checkers Require Import Cycle.
From BBB4.Checkers.IRules Require Import Expr RLE Engine Rules.
Import ListNotations.
Open Scope Z_scope.

(** ** Certificates *)

(** A template run: symbol, count [alpha * k + beta]. *)
Definition TRun : Set := (Sym * Z * Z)%type.

Definition tpl_start (rs : list TRun) : list SRun :=
  map (fun r => let '(s, al, be) := r in (s, mkExpr be [al])) rs.

(** The template one meta-cycle later: counts at [a*k + b]. *)
Definition tpl_want (a b : Z) (rs : list TRun) : list SRun :=
  map (fun r => let '(s, al, be) := r in
                (s, mkExpr (al * b + be) [al * a])) rs.

Record IRCert : Set := mkIRCert {
  c_anchor : nat;      (* anchor step: config index of C(k0) *)
  c_k0 : Z;
  c_kmin : Z;
  c_a : Z;
  c_b : Z;             (* meta map k -> a*k + b *)
  c_st : St;
  c_hs : Sym;
  c_TL : list TRun;    (* left template, nearest run first *)
  c_TR : list TRun;
  c_rules : list Rule
}.

Definition tpl_cfg (cert : IRCert) : SCfg :=
  mkSCfg (c_st cert) (c_hs cert)
         (tpl_start (c_TL cert)) (tpl_start (c_TR cert)).

Definition want_cfg (cert : IRCert) : SCfg :=
  mkSCfg (c_st cert) (c_hs cert)
         (tpl_want (c_a cert) (c_b cert) (c_TL cert))
         (tpl_want (c_a cert) (c_b cert) (c_TR cert)).

(** ** The checker *)

Fixpoint check_rules (tm : TM) (fuel : nat) (rs : list Rule)
  : option (list (Rule * list Tr)) :=
  match rs with
  | [] => Some []
  | r :: t =>
      match rule_check tm fuel r with
      | Some F => option_map (cons (r, F)) (check_rules tm fuel t)
      | None => None
      end
  end.

Definition st_in (q : St) (F : list Tr) : bool :=
  existsb (fun t => st_eqb (fst t) q) F.

Definition irules_check_neverqh (tm : TM) (cert : IRCert) (fuel : nat)
  : bool :=
  (0 <=? c_kmin cert) && (c_kmin cert <=? c_k0 cert) &&
  (0 <=? c_a cert) &&
  (c_kmin cert <=? c_a cert * c_kmin cert + c_b cert) &&
  match check_rules tm fuel (c_rules cert) with
  | None => false
  | Some rules =>
      match replay tm [c_kmin cert] rules
              (fun c => scfg_eqb c (want_cfg cert))
              fuel false (tpl_cfg cert) with
      | None => false
      | Some (_, F) =>
          match csteps tm (c_anchor cert) c0 with
          | Some (q, (l, h, r)) =>
              st_eqb q (c_st cert) && sym_eqb h (c_hs cert) &&
              lpad_eqb l (dside (fun _ => c_k0 cert)
                            (tpl_start (c_TL cert))) &&
              lpad_eqb r (dside (fun _ => c_k0 cert)
                            (tpl_start (c_TR cert))) &&
              forallb (fun q' =>
                         implb (cvisits tm c0 (c_anchor cert) q')
                               (st_in q' F)) all_St
          | None => false
          end
      end
  end.

(** ** Soundness *)

Lemma check_rules_sound : forall tm fuel rs rules,
  check_rules tm fuel rs = Some rules ->
  forall r F, In (r, F) rules -> rule_sem tm r F.
Proof.
  induction rs as [|r0 t IH]; intros rules H r F Hin; simpl in H.
  - injection H as <-. destruct Hin.
  - destruct (rule_check tm fuel r0) as [F0|] eqn:Hc; [|discriminate].
    destruct (check_rules tm fuel t) as [rules'|] eqn:Hcr;
      simpl in H; [|discriminate].
    injection H as <-.
    destruct Hin as [Heq | Hin].
    + injection Heq as <- <-. eapply rule_check_sound; eauto.
    + eapply IH; eauto.
Qed.

(** The one-meta-cycle denotation shift: [C(k)]'s successor template
    at [k] is [C(a*k + b)]. *)
Lemma tpl_shift : forall a b K rs,
  dside (fun _ => K) (tpl_want a b rs) =
  dside (fun _ => a * K + b) (tpl_start rs).
Proof.
  induction rs as [|[[s al] be] t IH]; [reflexivity|].
  simpl tpl_want. simpl tpl_start.
  rewrite !dside_cons, IH.
  f_equal. f_equal.
  unfold cnt. f_equal.
  unfold eval. cbn [dot e_c0 e_cf]. ring.
Qed.

Lemma want_shift : forall cert K,
  asem (fun _ => K) (want_cfg cert) =
  asem (fun _ => c_a cert * K + c_b cert) (tpl_cfg cert).
Proof.
  intros. unfold asem, dcfg, want_cfg, tpl_cfg.
  cbn [s_st s_hs s_L s_R].
  rewrite !tpl_shift. reflexivity.
Qed.

Lemma bge_kmin : forall kmin K,
  0 <= kmin -> kmin <= K -> bge [kmin] (fun _ => K).
Proof.
  intros kmin K H0 HK i.
  destruct i as [|i].
  - simpl. lia.
  - rewrite nth_overflow by (simpl; lia). lia.
Qed.

Lemma st_in_sound : forall q F,
  st_in q F = true -> exists t, In t F /\ fst t = q.
Proof.
  intros q F H.
  apply existsb_exists in H as (t & Hin & Hq).
  apply st_eqb_spec in Hq. eauto.
Qed.

Theorem irules_check_neverqh_sound : forall tm cert fuel,
  irules_check_neverqh tm cert fuel = true -> NeverQuasiHaltsSt tm.
Proof.
  intros tm cert fuel H.
  unfold irules_check_neverqh in H.
  apply andb_prop in H as [H Hrest].
  apply andb_prop in H as [H Hinward].
  apply andb_prop in H as [H Ha0].
  apply andb_prop in H as [Hk0 Hkk].
  apply Z.leb_le in Hk0, Hkk, Ha0, Hinward.
  destruct (check_rules tm fuel (c_rules cert)) as [rules|] eqn:Hcr;
    [|discriminate].
  destruct (replay tm [c_kmin cert] rules
              (fun c => scfg_eqb c (want_cfg cert))
              fuel false (tpl_cfg cert)) as [[cend F]|] eqn:Hrep;
    [|discriminate].
  destruct (csteps tm (c_anchor cert) c0) as [[q1 [[l1 h1] r1]]|]
    eqn:Hanch; [|discriminate].
  apply andb_prop in Hrest as [Hrest Hpre].
  apply andb_prop in Hrest as [Hrest HpadR].
  apply andb_prop in Hrest as [Hrest HpadL].
  apply andb_prop in Hrest as [Hq1 Hh1].
  apply st_eqb_spec in Hq1. apply sym_eqb_spec in Hh1.
  (* the anchor configuration is C(k0) *)
  assert (Hanchor : stepn tm (c_anchor cert) InitES =
                    Some (asem (fun _ => c_k0 cert) (tpl_cfg cert))).
  { rewrite <- lift_c0.
    rewrite (csteps_lift _ _ _ _ Hanch).
    f_equal.
    unfold asem, dcfg, tpl_cfg. cbn [s_st s_hs s_L s_R].
    rewrite !lift_cc, Hq1, Hh1.
    rewrite (lpad_eqb_lift _ _ HpadL), (lpad_eqb_lift _ _ HpadR).
    reflexivity. }
  (* one meta cycle, at every K >= kmin *)
  assert (Hcycle : forall K, c_kmin cert <= K ->
    exists n, (1 <= n)%nat /\
      Reach tm F n (asem (fun _ => K) (tpl_cfg cert))
                   (asem (fun _ => c_a cert * K + c_b cert)
                         (tpl_cfg cert))).
  { intros K HK.
    assert (Hb : bge [c_kmin cert] (fun _ => K))
      by (apply bge_kmin; lia).
    destruct (replay_sound tm [c_kmin cert] rules _ fuel false
                (tpl_cfg cert) cend F Hrep (fun _ => K) Hb)
      as (Hend & n & HR & Hpos).
    { intros r Fr c1 c2 Hin Happ.
      exact (rule_apply_sound tm [c_kmin cert] r Fr c1 c2 Happ
               (check_rules_sound tm fuel _ _ Hcr r Fr Hin)
               (fun _ => K) Hb). }
    exists n. split; [apply Hpos; reflexivity|].
    setoid_rewrite (scfg_eqb_asem cend (want_cfg cert)
                      (fun _ => K) Hend) in HR.
    setoid_rewrite (want_shift cert K) in HR.
    exact HR. }
  (* the meta map is inward *)
  assert (Hinw : forall K, c_kmin cert <= K ->
                 c_kmin cert <= c_a cert * K + c_b cert).
  { intros K HK. nia. }
  (* tiling: every index from the anchor on is inside some cycle *)
  assert (Htiles : forall i, exists N K,
    (c_anchor cert + i <= N)%nat /\ c_kmin cert <= K /\
    stepn tm N InitES = Some (asem (fun _ => K) (tpl_cfg cert)) /\
    forall m, (c_anchor cert <= m)%nat -> (m < N)%nat ->
      exists cm, stepn tm m InitES = Some cm /\ In (trans_of cm) F).
  { induction i as [|i IH].
    - exists (c_anchor cert), (c_k0 cert).
      split; [lia|]. split; [lia|]. split; [exact Hanchor|].
      intros m Hm1 Hm2. lia.
    - destruct IH as (N & K & HN & HK & Hstep & Hcov).
      destruct (Hcycle K HK) as (n & Hn1 & HS & HC & _).
      exists (N + n)%nat, (c_a cert * K + c_b cert).
      split; [lia|]. split; [apply Hinw; exact HK|]. split.
      + rewrite stepn_add, Hstep. exact HS.
      + intros m Hm1 Hm2.
        destruct (Nat.lt_ge_cases m N) as [Hlt | Hge].
        * exact (Hcov m Hm1 Hlt).
        * destruct (HC (m - N)%nat ltac:(lia)) as (cm & Hcm & Hin).
          exists cm. split; [|exact Hin].
          replace m with (N + (m - N))%nat by lia.
          rewrite stepn_add, Hstep. exact Hcm. }
  (* every transition in F fires unboundedly often *)
  assert (Hrec : forall t, In t F -> forall B,
    exists m cm, (B <= m)%nat /\ stepn tm m InitES = Some cm /\
                 trans_of cm = t).
  { intros t Hin B.
    destruct (Htiles B) as (N & K & HN & HK & Hstep & _).
    destruct (Hcycle K HK) as (n & Hn1 & _ & _ & HX).
    destruct (HX t Hin) as (m' & cm & Hm' & Hcm & Htr).
    exists (N + m')%nat, cm.
    split; [lia|]. split; [|exact Htr].
    rewrite stepn_add, Hstep. exact Hcm. }
  (* the never-quasihalting argument *)
  intros q Hvq B.
  (* q owns a transition in the cycle's fired set *)
  assert (Hqt : exists t, In t F /\ fst t = q).
  { destruct Hvq as (n0 & cn & Hcn & Hqn).
    destruct (Nat.lt_ge_cases n0 (c_anchor cert)) as [Hlt | Hge].
    - (* prefix: the checked cvisits subset condition *)
      destruct (stepn_csteps tm n0 cn Hcn) as (ccn & Hccn & Hlift).
      assert (Hfst : fst ccn = q).
      { destruct ccn as [qq tt]. simpl.
        rewrite <- Hlift in Hqn. exact Hqn. }
      assert (Hv : cvisits tm c0 (c_anchor cert) q = true)
        by (eapply cvisits_complete; eauto).
      rewrite forallb_forall in Hpre.
      specialize (Hpre q (all_St_complete q)).
      rewrite Hv in Hpre. simpl in Hpre.
      apply st_in_sound. destruct (st_in q F); [reflexivity|].
      discriminate.
    - (* from the anchor on: covered by the cycles *)
      destruct (Htiles (n0 + 1 - c_anchor cert)%nat)
        as (N & K & HN & HK & Hstep & Hcov).
      destruct (Hcov n0 Hge ltac:(lia)) as (cm & Hcm & Hin).
      rewrite Hcn in Hcm. injection Hcm as <-.
      exists (trans_of cn). split; [exact Hin|].
      unfold trans_of. simpl. exact Hqn. }
  destruct Hqt as (t & Hin & Hfst).
  destruct (Hrec t Hin B) as (m & cm & Hm & Hcm & Htr).
  exists m. split; [exact Hm|].
  exists cm. split; [exact Hcm|].
  rewrite <- Hfst, <- Htr. reflexivity.
Qed.

Corollary irules_check_nonhalt : forall tm cert fuel,
  irules_check_neverqh tm cert fuel = true -> NonHalt tm.
Proof.
  intros. eapply never_qh_nonhalt, irules_check_neverqh_sound; eauto.
Qed.
