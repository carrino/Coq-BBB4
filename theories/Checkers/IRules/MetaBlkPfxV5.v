(** * IRules.MetaBlkPfxV5: the Phase-2 block checker with the v5 end-match.

    A fork of [MetaBlkPfx.irulesblkpfx_check_neverqh] whose ONLY change
    is the meta-cycle closing test: it accepts a replayed configuration
    that is symbolic-cell-stream-equal to the shifted template
    ([StreamEq.bend_eqb2 = bend_eqb || cseq]), not only one recognised
    by the constant-count re-encoding of [bend_eqb].  This closes the
    "v5 rule-replay gap" -- the 45 BBB-verified list-C never-QH machines
    whose meta-cycle ends on a configuration that re-splits a
    variable-count block run against surrounding constant runs (the C
    verifier's [iv_streams_eq] accepts them; the landed Coq [bend_eqb]
    refuses them).

    Every other layer -- rule validation ([check_rulesBlkP]), the rule
    replay ([breplayKP]), the anchor re-simulation, the state-coverage
    argument -- is IDENTICAL to [MetaBlkPfx]; the soundness proof is the
    same, with [bend_eqb_bsem] replaced by [StreamEq.bend_eqb2_bsem] at
    the single point that discharges the cycle-closing equality.

    [Print Assumptions irulesblkpfx_check_neverqh_v5_sound] must be
...
(** ** The checker (endt uses [bend_eqb2]) *)

Definition irulesblkpfx_check_neverqh_v5 (tm : TM) (cert : BIRCertP)
    (cfuel fuel : nat) : bool :=
...
Theorem irulesblkpfx_check_neverqh_v5_sound : forall tm cert cfuel fuel,
  irulesblkpfx_check_neverqh_v5 tm cert cfuel fuel = true ->
  NeverQuasiHaltsSt tm.
Proof.
  intros tm cert cfuel fuel H.
  unfold irulesblkpfx_check_neverqh_v5 in H. cbv zeta in H.
...
Corollary irulesblkpfx_check_neverqh_v5_nonhalt : forall tm cert cfuel fuel,
  irulesblkpfx_check_neverqh_v5 tm cert cfuel fuel = true -> NonHalt tm.
Proof.
  intros. eapply never_qh_nonhalt, irulesblkpfx_check_neverqh_v5_sound; eauto.
Qed.
