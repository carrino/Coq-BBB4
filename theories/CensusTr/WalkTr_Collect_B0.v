(** * CensusTr/WalkTr_Collect_B0: shard B0 of the 4-way transition-level
    COLLECTION walk (subtree rooted at [child S0 DR StB]).
...
Time Definition walk_tr_out : (nat * nat) * list N :=
  Eval vm_compute in
    let q := Nat.iter WALK_TR_ITERS q_suc_tr (q_sub_tr S0 StB) in
