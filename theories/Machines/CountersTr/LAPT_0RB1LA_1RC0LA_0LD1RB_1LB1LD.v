(** * LAPT_0RB1LA_1RC0LA_0LD1RB_1LB1LD: TRANSITION-LEVEL board for machine 0RB1LA_1RC0LA_0LD1RB_1LB1LD, boarded by CERTIFICATE.
...
Definition pins_0RB1LA_1RC0LA_0LD1RB_1LB1LD : list Instr := [].
Definition tmw_0RB1LA_1RC0LA_0LD1RB_1LB1LD : TM := tm_wrap_trs tmm_0RB1LA_1RC0LA_0LD1RB_1LB1LD pins_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
Local Notation tm := tmw_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
...
Lemma fireo_0RB1LA_1RC0LA_0LD1RB_1LB1LD : forall (l : list lstep) (t : Instr),
  srun_instr tm true true l B0_0RB1LA_1RC0LA_0LD1RB_1LB1LD = Some t ->
  forall p j, cview p = (S j, None) ->
  exists k c, csteps tm k (Cc p) = Some c /\ cinstr c = t.
Proof.
  intros l t Hst p j E.
  apply (fire_of_run_instr tm Cc true true l B0_0RB1LA_1RC0LA_0LD1RB_1LB1LD p j [] []);
...
(** An instruction firing in the EXIT chain is reached from the outer overflow
    anchor by boot + the inner counter's own laps + that prefix. *)
Lemma firex_0RB1LA_1RC0LA_0LD1RB_1LB1LD : forall (l : list lstep) (t : Instr),
  srun_instr tm true true l BE0_0RB1LA_1RC0LA_0LD1RB_1LB1LD = Some t ->
  forall p j, cview p = (S j, None) ->
  exists k e, stepn tm k (lift (Cc p)) = Some e /\ instr_of e = t.
Proof.
  intros l t Hst p j E.
  apply (fire_via_fill tm Cc Cin lapin_0RB1LA_1RC0LA_0LD1RB_1LB1LD t p (pow2 j)).
  - exists (4 * j + 8), (cden [] [] j BB1_0RB1LA_1RC0LA_0LD1RB_1LB1LD). split;
      [| exact (gbo_0RB1LA_1RC0LA_0LD1RB_1LB1LD j)].
    rewrite (gso_0RB1LA_1RC0LA_0LD1RB_1LB1LD p j E).
    exact (srun_sound tm true true chb_0RB1LA_1RC0LA_0LD1RB_1LB1LD B0_0RB1LA_1RC0LA_0LD1RB_1LB1LD BB1_0RB1LA_1RC0LA_0LD1RB_1LB1LD 4 8
...
Lemma fire_0RB1LA_1RC0LA_0LD1RB_1LB1LD : forall t, ~ In t pins_0RB1LA_1RC0LA_0LD1RB_1LB1LD ->
...
Proof.
  ...
  - (* A0 *)
    ...
    apply (fireo_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 4; SCycR 2; SWinR 1] (StA, S0) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* A1 *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc Hi (StA, S1)).
    intros p1 j1 E1. apply (fire_lift_of_csteps tm Cc).
    apply (fireo_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 3] (StA, S1) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* B0 *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc Hi (StB, S0)).
    intros p1 j1 E1. apply (fire_lift_of_csteps tm Cc).
    apply (fireo_0RB1LA_1RC0LA_0LD1RB_1LB1LD [] (StB, S0) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* B1 *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc Hi (StB, S1)).
    intros p1 j1 E1. apply (fire_lift_of_csteps tm Cc).
    apply (fireo_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 2] (StB, S1) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* C0: fires in the exit half of the overflow *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc lapil_0RB1LA_1RC0LA_0LD1RB_1LB1LD (StC, S0)).
    intros p1 j1 E1.
    exact (firex_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 1; SWinL 2] (StC, S0) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* C1 *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc Hi (StC, S1)).
    intros p1 j1 E1. apply (fire_lift_of_csteps tm Cc).
    apply (fireo_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 1] (StC, S1) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* D0: fires in the exit half of the overflow *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc lapil_0RB1LA_1RC0LA_0LD1RB_1LB1LD (StD, S0)).
    intros p1 j1 E1.
    exact (firex_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 1; SWinL 5; SCycR 2; SWinR 1] (StD, S0) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
  - (* D1: fires in the exit half of the overflow *)
    apply (fire_csteps_of_lift tm Cc).
    apply (fire_via_ovf_lift tm Cc lapil_0RB1LA_1RC0LA_0LD1RB_1LB1LD (StD, S1)).
    intros p1 j1 E1.
    exact (firex_0RB1LA_1RC0LA_0LD1RB_1LB1LD [SCycL 2 0; SWin 1; SWinL 3] (StD, S1) ltac:(vm_compute; reflexivity)
                   p1 j1 E1).
Qed.

Theorem nqhtrm_0RB1LA_1RC0LA_0LD1RB_1LB1LD : NeverQuasiHaltsTr tmm_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
Proof.
  apply (glue_neverqhtr tmm_0RB1LA_1RC0LA_0LD1RB_1LB1LD pins_0RB1LA_1RC0LA_0LD1RB_1LB1LD Cc 1).
  - exact boot_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
  - intros p _. apply lap_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
  - intros t Ht p _. apply fire_0RB1LA_1RC0LA_0LD1RB_1LB1LD. exact Ht.
Qed.

Theorem nqhtr_0RB1LA_1RC0LA_0LD1RB_1LB1LD : NeverQuasiHaltsTr tm_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
Proof. apply (neverqhtr_mirror tm_0RB1LA_1RC0LA_0LD1RB_1LB1LD). rewrite mirror_ok_0RB1LA_1RC0LA_0LD1RB_1LB1LD. exact nqhtrm_0RB1LA_1RC0LA_0LD1RB_1LB1LD. Qed.

Theorem nonhalt_0RB1LA_1RC0LA_0LD1RB_1LB1LD : NonHalt tm_0RB1LA_1RC0LA_0LD1RB_1LB1LD.
Proof. apply never_qh_tr_nonhalt, nqhtr_0RB1LA_1RC0LA_0LD1RB_1LB1LD. Qed.
