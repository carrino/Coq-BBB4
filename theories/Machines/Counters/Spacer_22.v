(** * Spacer_22: the spacer_counter machine #22, 1RB1LA_1LC0RB_0LA0LD_1RA0RB.

    Second machine of the BBB harness's [spacer_counter] family
    (certificate results/counter22.cert: edge state B, voff -1,
...
Lemma U1 : wsteps true false tm_22 8 (StB, ([S0; S0], S1, []))
           = Some (StB, ([S1; S1], S1, [S1])).
Proof. reflexivity. Qed.

Lemma U2 : wsteps true false tm_22 9 (StB, ([S1; S1; S0], S1, [S1]))
           = Some (StC, ([S1; S0], S0, [S1; S1; S1])).
Proof. reflexivity. Qed.

Lemma U3 : wsteps true true tm_22 5 (StC, ([S1; S0], S0, []))
           = Some (StC, ([S1], S0, [S1])).
Proof. reflexivity. Qed.

Lemma U4 : wsteps true true tm_22 1 (StC, ([S1], S0, []))
           = Some (StA, ([], S1, [S0])).
Proof. reflexivity. Qed.

Lemma U5 : wsteps true true tm_22 1 (StA, ([S1], S1, []))
           = Some (StA, ([], S1, [S1])).
Proof. reflexivity. Qed.

Lemma U6 : wsteps true true tm_22 1 (StA, ([S0], S1, []))
           = Some (StA, ([], S0, [S1])).
Proof. reflexivity. Qed.

Lemma U7 : wsteps true true tm_22 1 (StA, ([], S0, [S1]))
           = Some (StB, ([S1], S1, [])).
Proof. reflexivity. Qed.

Lemma U8 : wsteps false true tm_22 3 (StA, ([S1], S1, [S1]))
           = Some (StB, ([S1], S1, [S1; S1])).
Proof. reflexivity. Qed.

Lemma U9 : wsteps true true tm_22 1 (StB, ([], S1, [S1]))
           = Some (StB, ([S0], S1, [])).
Proof. reflexivity. Qed.

Lemma U9b : wsteps true true tm_22 1 (StB, ([], S1, [S0]))
            = Some (StB, ([S0], S0, [])).
Proof. reflexivity. Qed.

Lemma U10 : wsteps true true tm_22 7 (StB, ([S0; S0; S0], S0, []))
            = Some (StB, ([S1; S1], S1, [S1])).
Proof. reflexivity. Qed.

(** Visit witnesses: C after 2 steps, A after 3, D after 6. *)
Lemma UV2 : wsteps true false tm_22 2 (StB, ([], S1, []))
            = Some (StC, ([], S0, [S1])).
Proof. reflexivity. Qed.

Lemma UV3 : wsteps true false tm_22 3 (StB, ([S0], S1, []))
            = Some (StA, ([], S0, [S0; S1])).
Proof. reflexivity. Qed.

Lemma UV6 : wsteps true false tm_22 6 (StB, ([S0; S0], S1, []))
...
Lemma lap_22 : forall p, (1 <= p)%positive ->
  exists n c', csteps tm_22 n (Cc p) = Some c' /\
...
Proof.
  ...
  replace (2 * S m + 4) with (S (S (S (S (S (S (2 * m)))))))
    by lia.
  replace (2 * S (S m) + 4) with (S (S (S (S (2 * m)))) + 4)
    by lia.
  cbn [rep app].
  destruct oq as [q0|].
  - (* interior carry *)
...
Lemma boot_22 : exists t0, stepn tm_22 t0 InitES = Some (lift (Cc 1)).
Proof.
  exists 78.
  assert (H : match csteps tm_22 78 c0 with
              | Some c => ceqb c (Cc 1)
              | None => false
              end = true) by (vm_compute; reflexivity).
  destruct (csteps tm_22 78 c0) as [c|] eqn:E; [|discriminate].
  rewrite <- lift_c0, (csteps_lift _ _ _ _ E).
  f_equal. apply ceqb_lift. exact H.
Qed.

Lemma vis_22 : forall p q, (1 <= p)%positive ->
  exists k c, csteps tm_22 k (Cc p) = Some c /\ fst c = q.
Proof.
  intros p q _.
  pose proof (Pos2Nat.is_pos p) as Hpos.
  unfold Cc.
...
(** #22 never quasihalts: bbchallenge 1RB1LA_1LC0RB_0LA0LD_1RA0RB. *)
Theorem nqh_1RB1LA_1LC0RB_0LA0LD_1RA0RB : NeverQuasiHaltsSt tm_22.
Proof.
  apply (glue_neverqh tm_22 Cc 1).
  - exact boot_22.
  - exact lap_22.
  - intros p q Hp. apply vis_22; exact Hp.
Qed.

Theorem tm_22_nonhalt : NonHalt tm_22.
Proof. apply never_qh_nonhalt, nqh_1RB1LA_1LC0RB_0LA0LD_1RA0RB. Qed.
