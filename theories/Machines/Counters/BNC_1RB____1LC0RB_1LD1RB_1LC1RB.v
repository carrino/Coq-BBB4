(** * BNC_1RB____1LC0RB_1LD1RB_1LC1RB: machine 1RB---_1LC0RB_1LD1RB_1LC1RB quasihalts, with bound 1.

    One of the three [1RB---] rows of the residue's `no anchor` bucket.
    [StA]'s [S1] transition is undefined, so [StA] fires once -- at
    configuration index 0 -- and [StB], [StC], [StD] are CLOSED
    ...
    counter is THE WALL POSITION, not a word:
      ...
      mkB j = (StB, (repeat S0 j ++ [S1], S0, []))

    -- the head on the blank frontier, [j] erased cells behind it, and the
    surviving wall cell at the far end.  [StB] sweeps right over ones
    writing zeros; [StC] and [StD] sweep back left over the zeros
...
Definition tm_1RB____1LC0RB_1LD1RB_1LC1RB : TM := fun q s => match q, s with
...
(** The wall-indexed anchor: [Cb StB p = mkB StB (Pos.to_nat p)]. *)
Lemma boot_1RB____1LC0RB_1LD1RB_1LC1RB : exists t0,
  stepn tm_1RB____1LC0RB_1LD1RB_1LC1RB t0 InitES = Some (lift (Cb StB 1)).
Proof.
  exists 4.
  assert (H : match csteps tm_1RB____1LC0RB_1LD1RB_1LC1RB 4 c0 with
              | Some c => ceqb c (Cb StB 1)
              | None => false
              end = true) by (vm_compute; reflexivity).
  destruct (csteps tm_1RB____1LC0RB_1LD1RB_1LC1RB 4 c0) as [c|] eqn:Eq; [|discriminate].
...
(** 1RB---_1LC0RB_1LD1RB_1LC1RB: never halts, quasihalts, and every quiet state made its last
    visit by index 1. *)
Theorem iqh_1RB____1LC0RB_1LD1RB_1LC1RB : iqh tm_1RB____1LC0RB_1LD1RB_1LC1RB.
Proof.
  apply (bounce_qh tm_1RB____1LC0RB_1LD1RB_1LC1RB StB StC StD
           eq_refl eq_refl eq_refl eq_refl eq_refl eq_refl
           boot_1RB____1LC0RB_1LD1RB_1LC1RB
