(** * BNC_0RB0LD_1LA1LC_0LD0LC_1RD1RB: machine 0RB0LD_1LA1LC_0LD0LC_1RD1RB never quasihalts.
    ...
    end at 3 steps per cell LEAVING THE TAPE UNCHANGED, and 4 more
    steps turn the walk around into the first bounce:

      wA n  -->  wM (n-2) 3   in  3*(n-1) + 5  steps.
...
Definition tm_0RB0LD_1LA1LC_0LD0LC_1RD1RB : TM := fun q s => match q, s with
...
Local Notation tm := tm_0RB0LD_1LA1LC_0LD0LC_1RD1RB.

Local Notation aA := (wA StB).
Local Notation aM := (wM StB).

Lemma chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall n1 n2 c c1 c2,
...
Definition mchain_0RB0LD_1LA1LC_0LD0LC_1RD1RB := wmchain tm StB StC StD
  eq_refl eq_refl eq_refl eq_refl eq_refl.
Definition visC_0RB0LD_1LA1LC_0LD0LC_1RD1RB := wvisC tm StB StC eq_refl.
Definition visD_0RB0LD_1LA1LC_0LD0LC_1RD1RB := wvisD tm StB StC StD eq_refl eq_refl eq_refl.

(** ** The collapse: [StA] walks the block at 3 steps per cell *)
...
Lemma rip1_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall L R,
  csteps tm 3 (StA, (S1 :: L, S1, S1 :: R))
    = Some (StA, (L, S1, S1 :: S1 :: R)).
Proof. intros L R. reflexivity. Qed.

Lemma ripn_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall k L R,
  csteps tm (3 * k) (StA, (repeat S1 k ++ L, S1, S1 :: R))
    = Some (StA, (L, S1, repeat S1 k ++ S1 :: R)).
Proof.
  induction k as [|k IH]; intros L R; [reflexivity|].
  cbn [repeat app].
  replace (3 * S k) with (3 + 3 * k) by lia.
  apply (chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB 3 (3 * k) _
           (StA, (repeat S1 k ++ L, S1, S1 :: S1 :: R))).
  - apply rip1_0RB0LD_1LA1LC_0LD0LC_1RD1RB.
  - rewrite IH, rep_snoc. reflexivity.
Qed.

(** The turn at the far end: the walk becomes the first bounce. *)
Lemma tail_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall X,
  csteps tm 4 (StA, ([], S1, S1 :: X))
    = Some (StB, ([S1; S1; S1], chd X, ctl X)).
Proof. intro X. reflexivity. Qed.

Lemma wcol_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall m,
  csteps tm (3 * (S m) + 5) (aA (S (S m))) = Some (aM m 3).
Proof.
  intro m. unfold wA.
  replace (3 * (S m) + 5) with (1 + (3 * (S m) + 4)) by lia.
  cbn [repeat].
  apply (chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB 1 _ _ (StA, (repeat S1 (S m), S1, [S1]))).
  { reflexivity. }
  apply (chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB (3 * (S m)) 4 _
           (StA, ([], S1, repeat S1 (S m) ++ [S1]))).
  { pose proof (ripn_0RB0LD_1LA1LC_0LD0LC_1RD1RB (S m) [] []) as Hr.
    rewrite app_nil_r in Hr. exact Hr. }
  rewrite rep_app1.
  exact (tail_0RB0LD_1LA1LC_0LD0LC_1RD1RB (repeat S1 (S m))).
Qed.

(** ** The macro lap *)

Lemma wlap_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall m, exists k, 0 < k /\
  csteps tm k (aA (S (S m))) = Some (aA (3 + 3 * m + 3)).
Proof.
  intro m. destruct (mchain_0RB0LD_1LA1LC_0LD0LC_1RD1RB m 3) as (k & Hk & Hrun).
  exists (3 * (S m) + 5 + k). split; [lia|].
  apply (chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB (3 * (S m) + 5) k _ (aM m 3)).
  - apply wcol_0RB0LD_1LA1LC_0LD0LC_1RD1RB.
  - exact Hrun.
Qed.

(** ** The anchor family: block sizes 3, 9, 27, ... *)

Fixpoint blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB (t : nat) : nat :=
  match t with 0 => 3 | S t' => 3 * blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB t' end.

Lemma blk_ge_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall t, 2 <= blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB t.
Proof. induction t; cbn; lia. Qed.

(** Every block the family visits is big enough to collapse. *)
Lemma anchor_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall t, exists m, blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB t = (S (S m)).
Proof.
  intro t. pose proof (blk_ge_0RB0LD_1LA1LC_0LD0LC_1RD1RB t).
  exists (blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB t - 2). lia.
Qed.

Definition Cc (p : positive) : cconf := aA (blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB (Pos.to_nat p)).

Lemma lap_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall p, exists n c',
  csteps tm n (Cc p) = Some c' /\ lift c' = lift (Cc (Pos.succ p)) /\ 0 < n.
Proof.
  intro p. unfold Cc. rewrite Pos2Nat.inj_succ. cbn [blk_0RB0LD_1LA1LC_0LD0LC_1RD1RB].
  destruct (anchor_0RB0LD_1LA1LC_0LD0LC_1RD1RB (Pos.to_nat p)) as (m & Hm). rewrite Hm.
  destruct (wlap_0RB0LD_1LA1LC_0LD0LC_1RD1RB m) as (k & Hk & Hrun).
  exists k, (aA (3 + 3 * m + 3)).
...
Lemma visA_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall m,
  exists c, csteps tm 1 (aA (S (S m))) = Some c /\ fst c = StA.
Proof.
  intro m. unfold wA. cbn [repeat]. eexists. split; reflexivity.
Qed.

Lemma vis_0RB0LD_1LA1LC_0LD0LC_1RD1RB : forall p q, exists k c,
  csteps tm k (Cc p) = Some c /\ fst c = q.
Proof.
  intros p q. unfold Cc.
  destruct (anchor_0RB0LD_1LA1LC_0LD0LC_1RD1RB (Pos.to_nat p)) as (m & Hm). rewrite Hm.
  destruct q.
  - destruct (visA_0RB0LD_1LA1LC_0LD0LC_1RD1RB m) as (c & Hc & Hq).
    exists 1, c. split; [exact Hc | exact Hq].
  - exists 0. eexists. split; reflexivity.
  - destruct (visC_0RB0LD_1LA1LC_0LD0LC_1RD1RB m 3) as (c & Hc & Hq).
    exists (3 * (S m) + 5 + 1), c. split; [| exact Hq].
    apply (chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB (3 * (S m) + 5) 1 _ (aM m 3)).
    + apply wcol_0RB0LD_1LA1LC_0LD0LC_1RD1RB.
    + exact Hc.
  - destruct (visD_0RB0LD_1LA1LC_0LD0LC_1RD1RB m 3) as (c & Hc & Hq).
    exists (3 * (S m) + 5 + (3 + 2)), c. split; [| exact Hq].
    apply (chain_0RB0LD_1LA1LC_0LD0LC_1RD1RB (3 * (S m) + 5) (3 + 2) _ (aM m 3)).
    + apply wcol_0RB0LD_1LA1LC_0LD0LC_1RD1RB.
    + exact Hc.
Qed.
...
Lemma boot_0RB0LD_1LA1LC_0LD0LC_1RD1RB : exists t0, stepn tm t0 InitES = Some (lift (Cc 1)).
Proof.
  exists 47.
  ...
  destruct (csteps tm 47 c0) as [c|] eqn:Eq; [|discriminate].
...
Theorem nonhalt_0RB0LD_1LA1LC_0LD0LC_1RD1RB : NonHalt tm.
Proof. apply never_qh_nonhalt, nqh_0RB0LD_1LA1LC_0LD0LC_1RD1RB. Qed.
