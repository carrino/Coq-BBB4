(** * B2L_1RB1LC_1LB1RA_0LC0LD_0RA0RD: the machine never quasihalts.
    ...
    CELL over 1,023 consecutive anchor visits with zero mismatches, values
    [1, 2, 3, ...] with no offset -- so [Cf : positive -> cconf] fits and the
    closer is the PLAIN [LapGlue.glue_neverqh].

    The lap is GEOMETRIC, [3.5*3^c + c + 2.5] in the carry length [c], which
    ...
    ** Why it is NEVER-quasihalting
    ...
    All four states occur in EVERY lap -- measured, every one of 1,022 laps
    of this row -- which is [glue_neverqh]'s [Hvis] premise.

    ** The one thing this row does not share with its partner

    [1RB1LC_1LC1RA_0LC0LD_0RA0RD] differs only in [B0]: [1LB] here, [1LC]
    there.  Every [B]-on-[S0] event of either row has an [S1] to its left
    (measured, 3,000,000 steps, 214,290 events here, zero exceptions), and
    from that shape both rows run the same composite

      (qB, (S1::L, S0, R))  ->  (qD, (ctl L, chd L, S0::S1::R))

    this row in FOUR steps ([B0] turns back into [qB], which finds the set
    cell, hands to [qA], which hands to [qC]) and its partner in two.  That
    is [Bin3Lap]'s [Hbc], discharged here by [Bin3Lap.bc_selfB]; nothing
    downstream sees the difference, so one closer serves both rows and the
    whole lap gap rides inside the existential.
...
Definition tm_1RB1LC_1LB1RA_0LC0LD_0RA0RD : TM := fun q s => match q, s with
...
Local Notation tm := tm_1RB1LC_1LB1RA_0LC0LD_0RA0RD.

(** The counter reads 1 after five steps: the blank tape's first three moves
    lay the low digit, and [B0]'s four-step composite brings the head home. *)
Lemma boot_1RB1LC_1LB1RA_0LC0LD_0RA0RD :
  stepn tm 5 InitES = Some (lift (Bin3Lap.Cf StD 1)).
Proof.
...
Lemma hbc_1RB1LC_1LB1RA_0LC0LD_0RA0RD : forall L R, exists n, 0 < n /\
  csteps tm n (StB,(S1::L,S0,R)) = Some (StD,(ctl L,chd L,S0::S1::R)).
Proof. apply (Bin3Lap.bc_selfB tm StA StB StC StD); reflexivity. Qed.

Lemma hbcC_1RB1LC_1LB1RA_0LC0LD_0RA0RD : forall L R, exists k c,
  csteps tm k (StB,(S1::L,S0,R)) = Some c /\ fst c = StC.
Proof. apply (Bin3Lap.bcC_selfB tm StA StB StC); reflexivity. Qed.

Theorem nqh_1RB1LC_1LB1RA_0LC0LD_0RA0RD : NeverQuasiHaltsSt tm.
Proof.
  exact (Bin3Lap.bin3_nqh tm StA StB StC StD
           eq_refl eq_refl eq_refl eq_refl eq_refl eq_refl eq_refl
           hbc_1RB1LC_1LB1RA_0LC0LD_0RA0RD
           hbcC_1RB1LC_1LB1RA_0LC0LD_0RA0RD
           ltac:(intro q; destruct q; auto)
           1%positive
           (ex_intro _ 5 boot_1RB1LC_1LB1RA_0LC0LD_0RA0RD)).
Qed.

Theorem nonhalt_1RB1LC_1LB1RA_0LC0LD_0RA0RD : NonHalt tm.
Proof. apply (never_qh_nonhalt tm nqh_1RB1LC_1LB1RA_0LC0LD_0RA0RD). Qed.
