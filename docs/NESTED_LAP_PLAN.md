...
`tools/counters/mpboot.py` reproduces the measurement.  The alphabet is now
in the zoo as **`Alph_01_11_011`** (`theories/Counters/Alph_01_11_011.v`,
generated by `gen_alphabet.py --abc 01,11,011`, so its two decomposition
lemmas are PROVED by induction, not asserted — a wrong triple fails to
compile).  Note it does NOT help the flat emitter: `emit_lapcert` still
derives 0 of 31, because these machines' overflow is genuinely exponential.
Its value is that it makes the INNER family decodable.

### The index-shift, and the one thing the step language cannot say

With the correct alphabet the inner start is `v0 = pow2 j + 1`, and

    E (2^j + 1) = B ++ rep A (j-1) ++ C

so the inner anchor's block count is **`j-1`** while the outer overflow
source's is **`j`** (`obS = 0`).  An `sside` carries `a*j + b` with `b : nat`,
so count `j-1` is `1*j + (-1)` — **not representable**, and no `lstep`
decrements a count: `SFold` only moves copies from the prefix INTO the count
(`b := b + m`), never out.

The fix is a REINDEX, not a new step.  Put the whole overflow branch at
`i = j - 1`:

    outer source   count j   = i + 1   ->  (a=1, b=1)   representable
    inner anchor   count j-1 = i       ->  (a=1, b=0)   representable

Both sides are then ordinary `sside`s and the existing `srun_sound` applies
unchanged.  The `j = 0` case (outer `p = fill (pow2 0) = 1`) drops out of the
reindex and needs its own chain — which is exactly the `j = 0` split
