...
3. it is **skipped**: one of the **37** undecided core machines in
   ...
   `python3 tools/closeout/audit.py` prints them live.  37 + 12 is the
   2026-07-31 reading (5,107 of the frozen 5,156 settled, 99.0%)._
...
1. **The champion's board cannot be CONSUMED as `boarded` is defined.**
   ...
   (which weakens `bbb4_decided_le_prev_champion` for all 5,107 boarded rows) or
   ...
   value is not proved.**
2. **The 37 core machines (and their 12 shadows).**  Any of them could, for all this
...
> 32,779,478 or never quasihalts, except 49 still-undecided machines (37
> core rows and their 12 0RB re-root shadows) —
> kernel-checked with one standard axiom.  The BBB(4) *value* does not yet
> follow from what is here, because the champion itself is one of the 37.

## Scope of the 37

All 37 core machines are residue — machines no engine in this repository
settles, mapped by
