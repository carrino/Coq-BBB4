...
3. it is **skipped**: one of the **35** undecided core machines in
   ...
   `python3 tools/closeout/audit.py` prints them live.  35 + 12 is the
   2026-07-31 reading (5,109 of the frozen 5,156 settled, 99.1%)._
...
1. **The champion's board cannot be CONSUMED as `boarded` is defined.**
   ...
   (which weakens `bbb4_decided_le_prev_champion` for all 5,109 boarded rows) or
   ...
   value is not proved.**
2. **The 35 core machines (and their 12 shadows).**  Any of them could, for all this
...
> 32,779,478 or never quasihalts, except 47 still-undecided machines (35
> core rows and their 12 0RB re-root shadows) —
> kernel-checked with one standard axiom.  The BBB(4) *value* does not yet
> follow from what is here, because the champion itself is one of the 35.

## Scope of the 35

All 35 core machines are residue — machines no engine in this repository
settles, mapped by
