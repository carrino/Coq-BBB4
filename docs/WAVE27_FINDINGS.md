...
**@NBOARD@ new boards, all first render except three, all funext-only.**
`D_remaining` **427 → @DREM@**.  Two new library files
...
## 2. The SKIP route (RESIDUE_PROMPT item 3): @NSKIP@ of the 211
...
* **The board** (`SKIP_*`): `Cc p = match pexp p with Some (S k) => VIRT k
  ...
  `4j+4`, fill `4j+6`... wait, measured split: fill `0j+1` + virt `4k+10`
  (the extractor picks the FIRST split that derives and validates; any
  valid split of the `4j+7` phase is sound, and the chains are re-run by
  the kernel either way).

Scoreboard on the 211: **@NSKIP@ boarded** (@NS1@ skip-1, @NS2@ skip-2,
...
* **@NREST@ rows** are the alternate-octave / register×counter regime
...
## 3. The QUAD route (RESIDUE_PROMPT item 1, WAVE26 7f): @NQUAD@ of the 41
...
Scoreboard on the 41: **@NQUAD@ boarded** — the single-class anchor-pivot
...
* **9 parity-class**: micro chains differ in LETTERS by k-parity; the
  emitter needs per-parity chain pairs and the `k = 2i+r` reindex
  (`a = 2` ssides — the checker already expresses them);
* **3 deep-pivot** (mode `(1, True)`), whose boot is a j-dependent law;
* **3 whose right sides are not `rep RU k ++ RPOST`** (2-cell stride
  variants).
...
other), with a floor for the run's own transient.  Over the @NREST@
unboarded rows: **@RESTPLAIN@ plain-full or skip under a new tail
(boarded @RESTBOARD@ of them through the flat/skip routes), @RESTOTHER@
"other"** — the register×counter regime: John's 8b read held on every
sampled row (the tape is `[register][1b-pairs]`; the family alternates
forms by octave, with register steps at measured mid-octave points).
