...
|---|---:|---|---|---|
| DN | 4,033 | dense: every instruction still firing at 1e8 | the RepWL finder with the block-length ladder takes only ~7% (3 of a random 40; the head of the list is easier, which misled the first estimate), so mostly the n-gram route at window 4–6, then whatever comes after it | `RW`, `NG` |
