MEASURE THE THREE REMAINING BLOCKED BUCKETS BEFORE BUILDING ANY OF THEM — **and diff `origin/main` before you pick one** — in `carrino/Coq-BBB4`, on branch `claude/ladder-gray-<yourid>` cut from main (§4l merged: both knobs on both arms, the quasihalt closer, nineteen boards, ten of them net; core undecided 47 → 43, remaining 68 → 58).
...
* **Diff `origin/main` before choosing a bucket.** The three-state counter wave boarded NINE of this session's nineteen rows concurrently (PR #91), so nineteen boards were ten rows. `git show origin/main:tools/closeout/core_rows.txt` is the one-diff check and it costs nothing.
