...
def rlbs(rr):    return [rc[2] for (_, rc) in rr if rc[0] == 'V']
...
def rend(vid, rr):
    ...
    for (s, rc) in rr:
        if rc[0] == 'C':
            ...
            out.append((s, eaddc(evar(vid), rc[1]))); vid += 1
    return out
...
def find_binding(lo, decs):
    for (delta, lb, e) in decs:
        ...
        if not (lb >= dj):
            continue
        Rex = eaddc(ediv(eaddc(e, -lb), dj), 1)
...
def parse_cert(path):
    ...
    for ln in open(path):
        ...
        if k in ("anchor_step", "k0", "kmin", "meta_a", "meta_b",
            ...
            # rulepfx RIDX L|R  (this side is a near-head prefix match)
            ridx = int(p[1])
            c["pfx"].setdefault(ridx, [False, False])
            c["pfx"][ridx][0 if p[2] == "L" else 1] = True
        elif k == "rulerun":
