...
def cons(A,m):
    ...
    k=len(A)
    if m<CAP:
        # ext == m exactly: cell at index m-1 is 1 (if visible), all beyond are 0
        for i in range(k):
            idx=k-1-i          # window index from head: 0..k-1
            if idx>m-1 and A[i]==1: return False
        if m>=1 and m-1<k and A[k-m]!=1: return False
        return True
    # m == CAP : ext >= CAP ; cells at index < CAP-1 unconstrained, but if a
    # visible cell at index >= CAP-1 is 1 that is consistent too.
    return True
...
def succs(tab,node,k):
    ...
    if d==1:   # DR: l'=w::l, h'=b_0, r'=ctl r
        dOL=w; dOR=-B_[0]
        dEL=(w if ml==0 else 1); ml2=(w if ml==0 else min(ml+1,CAP))
        dER=(0 if mr==0 else -1)
        mr2s=[0] if mr<=1 else ([mr-1] if mr<CAP else [CAP-1,CAP])
        A2=A[1:]+[w]
        for mr2 in mr2s:
            for x in (0,1):
                B2=B_[1:]+[x]
        ...
        dOL=-A[-1]; dOR=w
        dER=(w if mr==0 else 1); mr2=(w if mr==0 else min(mr+1,CAP))
        dEL=(0 if ml==0 else -1)
        ml2s=[0] if ml<=1 else ([ml-1] if ml<CAP else [CAP-1,CAP])
        B2=[w]+B_[:-1]
        for ml2 in ml2s:
            for x in (0,1):
                A2=[x]+A[:-1]
...
def orbit_nodes(tab,k,T=3_000_000):
    ...
    for _ in range(T):
        ...
        ml=min(ext(L),CAP); mr=min(ext(R),CAP)
