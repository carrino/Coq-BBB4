...
def main():
    ...
    for si, chunk in enumerate(stages):
        ...
        # [Lia] only where a row needs it -- the champion's bound gate is the
        # one [lia] in the generated stages, and adding the import
        # unconditionally would rewrite all 50-odd CB files for one of them.
        if any(k == 'iqhch' for _, k, _, _, _ in chunk):
            L.append('From Coq Require Import List Lia.')
        else:
    ...
    L.append('Proof.')
    L.append('  intro tm.')
    L.append('  destruct (census_boarded tm)')
    L.append('    as [H | [[H | [(Hnh & Hb & Hq) | (Hnh & Hb & Hq)]] | H]].')
    ...
    L.append('  - left. apply (qhbound_mono B_champ);')
    L.append('      [unfold B_champ, champion_score; lia | exact Hb].')
    ...
    L.append('')
    L.append('(** The community-facing reading, in previous-record terms.')
    L.append('')
    L.append('    Every machine the theorem decides either never quasihalts,')
    L.append('    or quasihalts by the PREVIOUS champion\'s score, or')
    L.append('    quasihalts by the CHAMPION\'s.  The third case is')
    L.append('    [boarded]\'s third disjunct and it is inhabited by exactly')
    L.append('    ONE census row -- the champion 1RB1LD_1RC1RB_1LC1LA_0RC0RD')
    L.append('    and its orbit, the single `iqhch` line of')
    L.append('    tools/closeout/frozen_map.tsv.  So among all known (4,2)')
    L.append('    machines only the champion exceeds the previous record,')
    L.append('    exactly as before it was boarded; what changed is that it')
    L.append('    is now DECIDED rather than skipped.')
    L.append('')
    L.append('    (That the third case has one row is UNTRUSTED bookkeeping,')
    L.append('    like the partition audit: [boarded]\'s third disjunct')
    L.append('    carries a bound, not an identity, so the kernel states the')
    L.append('    disjunction and not the census of who satisfies it.  This')
    L.append('    corollary was called [bbb4_decided_le_prev_champion] and')
    L.append('    had two disjuncts while the champion was skipped.) *)')
    L.append('Corollary bbb4_decided_le_prev_champion_or_champion : forall tm,')
    L.append('  ~ skipped D_remaining tm ->')
    L.append('  QHBound prev_champion_score tm \\/ NeverQuasiHaltsSt tm')
    L.append('  \\/ QHBound champion_score tm.')
    L.append('Proof.')
    L.append('  intros tm Hres.')
    L.append('  destruct (census_boarded tm)')
    L.append('    as [H | [[H | [(Hnh & Hb & Hq) | (Hnh & Hb & Hq)]] | H]].')
    ...
    L.append('  - right; right. apply (qhbound_mono B_champ);')
    L.append('      [unfold B_champ, champion_score; lia | exact Hb].')
