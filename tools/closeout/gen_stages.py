...
def main():
    ...
    L.append('    quasihalts, or is one of the %d SKIPPED residue machines'
             % len(remaining))
    L.append('    (tools/closeout/frozen_unproven.txt), still undecided.')
    if CHAMPION in remaining:
        L.append('')
    ...
    L.append('  - left. apply (qhbound_mono 2000);')
    L.append('      [unfold champion_score; lia | exact Hb].')
    ...
    L.append('Qed.')
    L.append('')
    L.append('(** The census tier fact the corollary below rests on: every')
    L.append('    machine the theorem does not skip has score <= 2000')
    L.append('    ([B_census], definitionally).  NOTE this is an artifact of')
    L.append('    where the census drew its line, not of the (4,2) landscape:')
    L.append('    the machines known to score between 2000 and the champion --')
    L.append('    the four previous champions, 2,512..66,349 -- are all')
    L.append('    currently in the SKIPPED list. *)')
    L.append('Lemma bbb4_decided_le_2000 : forall tm,')
    L.append('  ~ Deferred D_remaining tm ->')
    L.append('  QHBound 2000 tm \\/ NeverQuasiHaltsSt tm.')
    L.append('Proof.')
    L.append('  intros tm Hres.')
    L.append('  destruct (census_boarded tm) as [H | [[H | (Hnh & Hb & Hq)] | H]].')
    L.append('  - left; exact H.')
    L.append('  - right; exact H.')
    L.append('  - left; exact Hb.')
    L.append('  - destruct (Hres H).')
    L.append('Qed.')
    prev_horner = str(PREV_CHAMPION_SCORE)[0]
    for d in str(PREV_CHAMPION_SCORE)[1:]:
        prev_horner = '(%s)*10 + %s' % (prev_horner, d)
    L.append('')
    L.append('(** The PREVIOUS champion\'s score, %s'
             % '{:,}'.format(PREV_CHAMPION_SCORE))
    L.append('    (1RB0LD_1LC0LA_1LA0LC_1RD1RC, theories/Machines/Counters/')
    L.append('    BlankTail_66349.v).  Digit form for the same [lia] reason')
    L.append('    as [champion_score]. *)')
    L.append('Definition prev_champion_score : nat :=')
    L.append('  %s.' % prev_horner)
    L.append('')
    ...
    L.append('    quasihalts -- so among all known (4,2) machines, only the')
    ...
    L.append('Proof.')
    L.append('  intros tm Hres.')
    L.append('  destruct (bbb4_decided_le_2000 tm Hres) as [H | H];')
    L.append('    [left | right; exact H].')
    L.append('  apply (qhbound_mono 2000);')
    L.append('    [unfold prev_champion_score; lia | exact H].')
    L.append('Qed.')
    L.append('')
