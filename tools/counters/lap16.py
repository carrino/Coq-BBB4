#!/usr/bin/env python3
"""Symbolic lap for #16 (1RB0RC_1RC1LB_1LD0RC_0LB0LA), spacer_counter,
edge C, voff 0, zoff 2.  Anchor S(m) = B(m) 11 0^(2m+2) 1(head), state C.
cconf: (C, rep [0] (2m+2) ++ [1,1] ++ bitsLSB(m), 1, [])."""
import sys
sys.path.insert(0, '.')
from executor import Exec, Wall, rep, norm, LAB

SPEC = "1RB0RC_1RC1LB_1LD0RC_0LB0LA"
A, B, C, D = 0, 1, 2, 3
...
def S_conf(m):
    return (C, [0] * (2 * m + 2) + [1, 1] + bitsL(m), 1, [])
...
def lap(ex, m):
    ...
    q, l, h, r = cfg
    assert l[:1] == [1] and l[1:z-1] == [0]*(z-2), f"transcription l shape"
    # apply manually (cycLW): unit (D,([1,0],0,[])) -> (D,([1],0,[1]))
    out = ex.wsteps(True, True, D, [1, 0], 0, [], 5)
    assert out == (D, [1], 0, [1]), f"U3 unit: {out}"
    ex.record("U3", True, True, 5, (D, (1, 0), 0, ()), (D, (1,), 0, (1,)))
    assert cfg[0] == D and cfg[2] == 0
    ex.steps += 5 * (z - 2)
    cfg = (D, [1] + l[z-1:], 0, [1] * (z - 2) + r)
    # glue: (D,([1],0,[])) -> (B,([],1,[0]))
...
def raw_lap(ex, m):
    ...
    for n in range(1, 3000 * m + 40000):
        cfg = ex.cstep(cfg)
        q, l, h, r = cfg
        if q == C and h == 1 and all(x == 0 for x in r):
            m2 = m + 1; z2 = 2 * m2 + 2
            ln = list(l)
