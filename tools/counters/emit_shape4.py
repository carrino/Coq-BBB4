...
def print_assumptions(ID, scratch):
    ...
    with open(chk, 'w') as f:
        f.write('From BBB4.Machines.Counters Require Import ILS4_%s.\n'
...
def process(spec, do_emit, scratch, force=False, windows=False):
    ...
    try:
        ...
        if exact_ov:
            raise DeriveError('exact-overflow variant not templated -- report')
    ...
    res.update({'edge': edge, 'tail': list(tail), 'p0': p0, 'boot': boot,
                ...
                'QP': LAB[QP], 'QR': LAB[QR],
                'deep': {LAB[q]: (w, t) for q, (w, t, _) in deep.items()}})
    if windows:
    ...
    if not do_emit:
        ...
        return res
    ID = mach_id(spec)
    path = os.path.join(OUTDIR, 'ILS4_%s.v' % ID)
